...
public abstract class AbsRecyclerViewFastScroller extends FrameLayout implements RecyclerViewScroller {
    ...
    */
    ...
    @Override
    public void setRecyclerView(RecyclerView recyclerView, SwipeRefreshLayout pullMe) {
        ...
        if (mOnScrollListener == null) {
            mOnScrollListener = new OnScrollListener() {
                ...
                public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                    ...
                    //}
                    if(thePuller != null && recyclerView != null && recyclerView.getChildCount() > 0){
                        try {
                            if (theTop < 0){
                                theTop = recyclerView.getChildAt(0).getTop();
                            }
                            boolean enable = false;
                            boolean firstItemVisiblePull = recyclerView.getChildPosition(recyclerView.getChildAt(0)) == 0;
                            boolean topOfFirstItemVisiblePull = recyclerView.getChildAt(0).getTop() == theTop;
                            enable = firstItemVisiblePull && topOfFirstItemVisiblePull;
                            thePuller.setEnabled(enable);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        /*
        Log.d("V1", mRecyclerView.getAdapter().getItemCount() + "");
        Log.d("V1", ((LinearLayoutManager) mRecyclerView.getLayoutManager())
    ...
    public void scrollHider(){
        if (mRecyclerView != null && mRecyclerView.getAdapter() != null) {
            if (mRecyclerView.getAdapter().getItemCount() == (((LinearLayoutManager) mRecyclerView.getLayoutManager())
                    .findLastCompletelyVisibleItemPosition() + 1)) {
                if (thePuller != null) {
                    thePuller.setEnabled(true);
                }
